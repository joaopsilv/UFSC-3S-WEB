...
createBtn.addEventListener('click', () => {
    ...
    let cepValido = buscaCEP(camposRegistro[6], 'statusCEP', 'CEP')

    let contaValida = nomeValido && sobrenomeValido && emailValido && senhaValida && cepValido
    if (contaValida){
        let conta = new Conta(camposRegistro[0].value, camposRegistro[1].value, cpfValido, camposRegistro[3].value, camposRegistro[4].value, camposRegistro[6].value)
        console.log(conta)
    }
})
...
function validaCEP(input, p, label){
    campoPreenchido = validaTextoEmBranco(input, p, label)
    p = document.getElementById(p)
    let cepCase = /^[0-9]{8}$/
    if (campoPreenchido){
        if (input.value.replace(/\D/g, '').match(cepCase)){
            p.style.color = 'green'
            p.innerText = `Válido!`
            return true
        } else{
            p.style.color = 'red'
            p.innerText = `O valor de ${label} precisa ser no padrão: 00000-000`
        }
    } else{
        p.style.color = 'red'
        p.innerText = `O valor de ${label} não pode ser nulo`
...
function buscaCEP(input, p, label){
    cepValido = validaCEP(input, p, label)
    if (cepValido){
        cep = input.value.replace(/\D/g, '')
        fetch(`https://viacep.com.br/ws/${cep}/json/`)
        .then(data => data.json())
        .then(res => {
            camposRegistro[7].value = res.logradouro
            camposRegistro[8].value = res.bairro
            camposRegistro[9].value = res.localidade
            camposRegistro[10].value = res.uf
        })
        return true
    }
    return false
...
class Conta{
    constructor(nome, sobrenome, cpf, email, senha, cep){
        this.nome = nome
